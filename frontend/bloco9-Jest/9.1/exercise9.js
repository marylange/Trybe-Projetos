...
const authorWith3DotsOnName = (livrosParaBusca) => {
  const regexEncontrarAutor = /^[A-Z]\. [A-Z]\. [A-Z]\./i;
  const livroEncontrado = livrosParaBusca.find((book) => regexEncontrarAutor.test(book.author.name));
  return livroEncontrado.name;
}
