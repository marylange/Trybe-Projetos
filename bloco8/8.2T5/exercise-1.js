...
const fantasyOrScienceFictionAuthors = (arr) => {
    const filtrandoGeneroDosFilmes = arr.filter((genero) => genero.genre == 'Ficção Científica' || genero.genre == 'Fantasia');
    const result = filtrandoGeneroDosFilmes.map((nomeAuthor) => nomeAuthor.author.name).sort();
    return result;
}
